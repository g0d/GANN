    ...
    if (SimFunc == 0)
    {
        ...
        for (n = 0; n < SimNum; n++)
        {
            ...
            if (NewGANN.Layers > 1 AND EpC > 1)
                LHP = (LHT + NewGANN.GMap[0]) - (NewGANN.GMap[StartLayer] + NewGANN.GMap[StartLayer - 1]);
            else
                LHP = 0;
            ...
            for (i = StartLayer; i < NewGANN.Layers; i++)
            {
                ...
                if (NewGANN.GMCType == 0)
                {
                    ...
                    for (j = LEP; j < (k + LEP); j++)
                    {
                        ...
                        if (i == 0)         //Get Data From InSocket (First Layer)
                        {
                            for (p = 0; p < Inputs; p++)
                            {
                        ...
                        else                //Get Data From Previous Layer
                        {
                            //Give Data To This GNeuron
            ...
            if (SimMode == 0)               //Learn
            {
                ...
                else
                {
                    ...
                    for (i = LHP; i < (LHP + LEP); i++)
                    {
                        ...
                        if ((GNet[i].OutData() * GMin) < GMin OR (GNet[i].OutData() * GMax) > GMax)
                        {
                ...
                for (j = LEP; j < (k + LEP); j++)
                {
                    ...
                    if (i == 0)
                    {
                        for (p = 0; p < Inputs; p++)
                        {
                            ...
                            printf("Weight %i: %f\n", p - LHP + 1, GNet[GNC].GetWeights(p - LHP));
                        }
                    }
                    else
                    {
